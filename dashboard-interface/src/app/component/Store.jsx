...
const BrandList = ({ brands, selectedBrandIndex, handleBrandClick }) => (
  <ul className='flex flex-row w-full items-center'>
    {brands.map((brand, index) => (
      <li
        key={brand.id}
        className={`text-gray-600 text-sm px-2 ${selectedBrandIndex === index ? 'text-blue-500' : ''}`}
...
const ProductList = ({ products, selectedProductIndex, handleProductClick }) => (
  <ul className='flex flex-row w-full items-center'>
    {products.map((product, index) => (
      <li
        key={product.id}
        className={`text-gray-600 text-sm px-2 ${selectedProductIndex === index ? 'text-blue-500' : ''}`}
  ...
  return (
    <div className='flex flex-col bg-white p-4 m-4 rounded-lg shadow-lg'>
      <div className='flex flex-row w-full justify-end items-center'>
        <div>
          <h2 className='text-gray-700 px-2'>{data.city}, {data.store}</h2>
        </div>
        <div>
